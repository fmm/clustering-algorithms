...
Algoritmo::Algoritmo(size_t inicializacoes, size_t clusters, size_t q, size_t limite, double alpha, const Repositorio& repositorio, ostream& out) :
  inicializacoes(inicializacoes), clusters(clusters), prototipos(prototipos), repositorio(repositorio), out(out) {
    melhor = atual = Resultado(alpha,clusters, repositorio.tabela[0].n, q, 0);
...
void Algoritmo::executar() {
  ...
  for (size_t i = 0; i < n; i++) {
    ...
    Array<double> us;
    for(size_t j=0;j<this->melhor.cluster.size();j++)
      us.push_back(this->melhor.U[j][i]);
    print2(this->out, i, k, us, c[i], p[i]);
  }
  printLinha1(this->out, k);
  this->out << "\n";
  // parte 3
  ...
  for (size_t i = 0; i < this->melhor.W.size(); i++) {
    this->out << "Cluster #" << (i+1) << "\n";
    for(size_t j = 0; j < this->melhor.W[i].size(); j++) {
      this->out << "\ttabela[" << j << "] : " << this->melhor.W[i][j] << "\n";
    }
  }
  this->out << "\n";
...
bool Algoritmo::etapa2() {
  if(!this->atual.atualizarUW(this->repositorio)) {
    return false;
  }
