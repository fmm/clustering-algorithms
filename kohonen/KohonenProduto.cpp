...
namespace Kohonen {
	...
	double func(int x, int y) {
		...
		rep(j,p) {
			#ifdef FUNC_L1
				dist += lambda[y][j] * (fabs(individual[x][j].first - prototype[y][j].first) + fabs(individual[x][j].second - prototype[y][j].second)); 
			#elif  FUNC_L2
				dist += lambda[y][j] * (pow(fabs(individual[x][j].first - prototype[y][j].first),2.0) + pow(fabs(individual[x][j].second - prototype[y][j].second),2.0));
			#endif
		}
		return dist;
	}
	...
	double ternarySearch(vector< double > &a, vector< double >& b, int maxIter = 200, double inf = 1e9, double eps = 1e-12) { // a[i] - b[i]*x
		...
		rep(iter,maxIter) {
			...
			#elif  FUNC_L2
				rep(i,n) f1 += b[i] * pow(fabs(a[i] - med1),2.0);
			#endif
			long double med2 = (low + high * 2) / 3, f2 = 0;
			#ifdef FUNC_L1
				rep(i,n) f2 += fabs(a[i] - b[i] * med2);
			#elif  FUNC_L2
				rep(i,n) f2 += b[i] * pow(fabs(a[i] - med2),2.0);
			#endif
	...
	void update(double temperature) {
		...
		rep(c,k) rep(j,p) {
			...
			#ifdef FUNC_L1
				rep(i,n) a[i] = individual[i][j].first * b[i];
			#elif  FUNC_L2
				rep(i,n) a[i] = individual[i][j].first;
			#endif
			prototype[c][j].first = ternarySearch(a,b);
			#ifdef FUNC_L1
				rep(i,n) a[i] = individual[i][j].second * b[i];
			#elif  FUNC_L2
				rep(i,n) a[i] = individual[i][j].second;
			#endif
			prototype[c][j].second = ternarySearch(a,b);
		}
	}
	...
	void fixLocal(double temperature) {
		...
		rep(c,k) rep(j,p) rep(i,n) {
			#ifdef FUNC_L1
				double g = fabs(individual[i][j].first - prototype[c][j].first) + fabs(individual[i][j].second - prototype[c][j].second);
				f[c][j] += kernel(who[i],c,temperature) * g;
			#elif  FUNC_L2
				double g = pow(fabs(individual[i][j].first - prototype[c][j].first),2.0) + pow(fabs(individual[i][j].second - prototype[c][j].second),2.0);
				f[c][j] += kernel(who[i],c,temperature) * g;
			#endif
		}
		vector< double > num(k, 1.0);
		rep(c,k) rep(j,p) {
			num[c] *= pow(f[c][j], 1.0 / p);
		}
		//double all = 1.0;
		rep(c,k) rep(j,p) {
			lambda[c][j] = num[c] / f[c][j];
		}
		//cerr << all << endl;
	}
	
	void fixGlobal(double temperature) {
		vector< double > f = vector< double > (p, 0);
		rep(c,k) rep(i,n) rep(j,p) {
			#ifdef FUNC_L1
				double g = fabs(individual[i][j].first - prototype[c][j].first) + fabs(individual[i][j].second - prototype[c][j].second);
				f[j] += kernel(who[i],c,temperature) * g;
			#elif  FUNC_L2
				double g = pow(fabs(individual[i][j].first - prototype[c][j].first),2.0) + pow(fabs(individual[i][j].second - prototype[c][j].second),2.0);
				f[j] += kernel(who[i],c,temperature) * g;
			#endif
